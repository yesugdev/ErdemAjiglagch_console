...
class ExcelAnalyzerGUI:
    def __init__(self, root):
        ...
        # Header
        self.header = ttk.Label(self.main_frame, text="Excel Data Analyzer", style='Header.TLabel')
    ...
    def create_file_tab(self):
        ...
        self.notebook.add(self.file_tab, text="1. Load Excel File")
        
        frame = ttk.Frame(self.file_tab)
        frame.pack(pady=20)
        
        ttk.Label(frame, text="Excel File Path:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        load_btn = ttk.Button(self.file_tab, text="Load Excel File", command=self.load_workbook)
        load_btn.pack(pady=10)
        
        self.sheet_info = tk.Text(self.file_tab, height=10, width=80, state=tk.DISABLED)
        self.sheet_info.pack(pady=10)
        
    def create_sheet_tab(self):
        self.sheet_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.sheet_tab, text="2. Select Sheet & Data", state=tk.DISABLED)
        
        frame = ttk.Frame(self.sheet_tab)
        frame.pack(pady=20)
        
        ttk.Label(frame, text="Select Sheet:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        ttk.Label(frame, text="Report Type:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        ttk.Label(frame, text="Data Range (e.g., A1:L5):").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
        self.range_entry = ttk.Entry(frame)
        self.range_entry.grid(row=2, column=1, padx=5, pady=5)
        
        self.preview_btn = ttk.Button(frame, text="Preview Data", command=self.preview_data, state=tk.DISABLED)
        self.preview_btn.grid(row=3, column=0, columnspan=2, pady=10)
        
        self.data_preview = tk.Text(self.sheet_tab, height=10, width=80, state=tk.DISABLED)
        self.data_preview.pack(pady=10)
        
        self.generate_btn = ttk.Button(self.sheet_tab, text="Generate Report", command=self.generate_report, state=tk.DISABLED)
        self.generate_btn.pack(pady=10)
        
    def create_report_tab(self):
        self.report_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.report_tab, text="3. View & Save Report", state=tk.DISABLED)
        
        self.report_text = scrolledtext.ScrolledText(self.report_tab, wrap=tk.WORD, width=100, height=30)
    ...
    def load_workbook(self):
        ...
        try:
            self.update_status("Loading workbook...")
            self.workbook = openpyxl.load_workbook(file_path)
            
            # Update sheet info
            self.sheet_info.config(state=tk.NORMAL)
            self.sheet_info.delete(1.0, tk.END)
            self.sheet_info.insert(tk.END, f"Successfully loaded: {file_path}\n")
            self.sheet_info.insert(tk.END, f"Available sheets: {', '.join(self.workbook.sheetnames)}")
    ...
    def preview_data(self):
        ...
        if not cell_range:
            messagebox.showerror("Error", "Please enter a data range")
            return
        ...
        if not self.selected_sheet or not self.report_type:
            messagebox.showerror("Error", "Please select both sheet and report type")
            return
    ...
    def generate_report(self):
        ...
        try:
            openai.api_key = "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"  # Replace with your actual API key
    ...
    def save_report(self):
        ...
        default_filename = f"{self.report_type}_report_{timestamp}.txt"
